	...
	public class LinqTests
	{
			...
			where TE : class, IEnumerable
		{
			...
			var result = await ParserFixture.CreateAndParseWithOptions(template, new Dictionary<string, object>
				{
					{ "data", data }
				}, opt, options => { return	options.WithFormatters(typeof(DynamicLinq)); },
				info => { info.CaptureVariables = true; },
				documentResult => { variables = documentResult.CapturedVariables; });
		...
		public async Task TestWhere()
		{
			...
			var simple = await CreateAndExecute<IEnumerable<string>, IEnumerable<string>>(
				"{{#var result = data.Where('it.StartsWith(\"A\")')}}", sl, _options);
			Assert.That(simple, Contains.Item("ABC"));
			Assert.That(simple, Contains.Item("ACB"));
			Assert.That(simple, Is.Not.Contain("CBA"));
			var withArg = await CreateAndExecute<IEnumerable<string>, IEnumerable<string>>(
				"{{#var result = data.Where('it.StartsWith(@0)', 'A')}}", sl, _options);
